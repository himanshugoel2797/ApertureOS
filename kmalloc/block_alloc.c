...
void SetRuntimeInitialized(bool status) { _runtime_initialized = status; }
                   ...
                   size_t blockSize)
{
	...
	for (UID i = 1; i < blockCount; i++)
	{
		if (blockData[blockIndex + i] != BLOCK_TYPE_FREE)
		{
			//If we don't try again
			return _allocBlockSet(blockIndex + i, blockCount, blockData, blockEntries, baseLoc, blockSize);
		}
	}
