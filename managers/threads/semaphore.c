...
bool
ThreadMan_WaitAcquireSemaphore(UID id)
{
    while(1)
        {
            ...
            if(!Interrupts_IsInHandler())ThreadMan_Yield();	//TODO this is gonna be a problem, create a means to register a function to be called immediately after current interrupt
            else return FALSE;
        }
}
