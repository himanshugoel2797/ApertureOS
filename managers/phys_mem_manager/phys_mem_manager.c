...
uint32_t pmem_Initialize() {
        ...
        KB4_Blocks_FreeBitCount =
                bootstrap_malloc(KB4_Blocks_FreeBitCount_EntryNum * sizeof(uint32_t));
...
void *MemMan_Alloc(uint64_t size) {
        ...
        for (; i < KB4_Blocks_Count; i++) {
                if (pageCount == 1) {
                        ...
                        if (KB4_Blocks_Bitmap[i] == 0) {
                                ...
                                // asm ("bsf %1, %0" : "=r" (numFreePages) : "r"
                                // (KB4_Blocks_Bitmap[i]));
                                uint32_t numFreePages = 0;
                                int z = 0;
                                for (; z < 32; z++) {
                                        if (!CHECK_BIT(KB4_Blocks_Bitmap[i], z))
                                                numFreePages++;
                        ...
                        int sections = pageCount / KB4_MAX_VAL;
                        if(pageCount % KB4_MAX_VAL != 0)sections++;
                        uint32_t base = 0, curBase = 0;
                        COM_WriteStr("Start!! Sections: %d\r\n", pageCount);
                        for (int n = 0; n < sections; n++) {
                                curBase = MemMan_Alloc(PAGE_SIZE * 32);
                                  COM_WriteStr("curbase: %x\r\n", curBase);
                                if (n == 0)
                                        base = curBase;
        ...
        if (i >= KB4_Blocks_Count) {
                COM_WriteStr("Failed to allocate memory!");
                return NULL;
        }
...
void *physMemMan_Alloc(uint64_t size) {
        ...
        // Allocate the next set on the second to last entry so the last entry is used
        // to track the next page
